...
- 6130fec: Expand newly added comment cards immediately and restore the Line spacing toolbar aria-label for e2e and a11y parity.
...
- 6130fec: Persist empty-paragraph formatting through focus/selection churn and keep the toolbar in sync with stored marks. Mark toggles rebind to the editor schema (and match by name) so bold/italic turn off correctly when duplicate ExtensionManager copies are loaded. Fixes empty-run bold/italic/font toggles being lost after Enter or delete-and-retype.
...
- 6130fec: Keep Decrease Indent enabled after list indent: do not clear toolbar formatting when the editor view is briefly unavailable, and derive list level from live ProseMirror state.
- 6130fec: Keep the off-screen body ProseMirror portal SSR-safe: skip `document.body` access during server render while still portaling to `document.body` in the browser.
- 6130fec: Make suggesting-mode Enter/Backspace create paragraph-mark revisions again, and stamp data-revision-author on painted tracked runs.
- 6130fec: Keep the caret in place when toggling marks from the toolbar while the editor still has focus, so typed formatting no longer jumps to a stale selection.
- 6130fec: Hold document parse under the Vite demo `?e2e=1&loading=1` flag so visual regression can snapshot real loading chrome without races.
